...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        header('location: ' . ROOT_URL . '/voiture.php');
        exit();
    }
    if (isset($_POST['car_id'], $_POST['name'], $_POST['email'], $_POST['phone_number'], $_POST['message'])) {
        $carId = $_POST['car_id']; // Récupérer l'ID de la voiture depuis les données POST
        ...
        if (empty($name) || empty($email) || empty($phone_number) || empty($message)) {
            ...
            // Insértion du message dans la base de données
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                header('Location: ' . ROOT_URL . '/voiture.php?id=' . $carId);
